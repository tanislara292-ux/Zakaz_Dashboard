...
echo "🚀 ЗАПУСК ПОЛНОГО ТЕСТИРОВАНИЯ Q TICKETS API"
echo "=============================================="

# Проверка наличия токена
if [ -z "$1" ]; then
    echo "❌ ОШИБКА: Не указан API токен"
    echo "Использование: ./run_tests.sh YOUR_API_TOKEN"
    echo "Пример: ./run_tests.sh abc123def456ghi789"
    exit 1
fi

TOKEN="$1"
echo "✅ Токен получен: ${TOKEN:0:10}..."
...
if ! command -v jq &> /dev/null; then
    echo "⚠️  ВНИМАНИЕ: jq не установлен. Рекомендуется установить для полного анализа"
    echo "Установка: brew install jq (macOS) или apt-get install jq (Linux)"
fi

# Запуск основного скрипта
echo "🔥 Запускаем полное тестирование..."
chmod +x full_test_execution.sh
./full_test_execution.sh "$TOKEN"

echo ""
echo "🎉 Тестирование завершено! Проверьте отчеты в директории test_results/"
```

Теперь я создам основной файл для выполнения всех тестов с детальным логированием:
<tool_call>thinking
<think></think>
```bash
#!/bin/bash
...
Теперь я создам PowerShell скрипт для Windows, так как вы работаете в Windows:
