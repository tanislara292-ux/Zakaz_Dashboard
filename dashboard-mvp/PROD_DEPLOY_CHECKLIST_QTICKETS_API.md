...
- [ ] Проверить, что таймер работает по расписанию (подождать 30 минут):

```bash
# Через 30 минут после включения таймера
journalctl -u qtickets_api.service --since "30 minutes ago" --no-pager
```
...
✅ **Данные поступают каждые 30 минут (с однократным бэком за 30 дней при первом запуске)**
...
Можем сказать заказчику: "данные с вашего билета-продавца QTickets прилетают к вам в ClickHouse каждые 30 минут, DataLens читает витрины, дашборд живёт".
