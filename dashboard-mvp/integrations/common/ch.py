...
class ClickHouseClient:
    ...
    def __init__(self, host: str = None, port: int = None, 
                 ...
                 database: str = None, secure: bool = True,
    ...
    def _connect(self):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.info(f"Успешное подключение к ClickHouse: {self.host}:{self.port}")
                return
            except Exception as e:
                logger.warning(f"Попытка {attempt + 1}/{self.max_retries} подключения к ClickHouse не удалась: {e}")
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay * (2 ** attempt))  # Экспоненциальный бэкофф
                else:
                    logger.error(f"Не удалось подключиться к ClickHouse после {self.max_retries} попыток")
                    raise
    ...
    def execute(self, query: str, parameters: Optional[Dict[str, Any]] = None) -> Any:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"Попытка {attempt + 1}/{self.max_retries} выполнения запроса не удалась: {e}")
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay)
                    # Попытка переподключения
                    self._connect()
                else:
                    logger.error(f"Не удалось выполнить запрос после {self.max_retries} попыток: {query[:100]}...")
                    raise
            except Exception as e:
    ...
    def insert(self, table: str, data: List[Dict[str, Any]], 
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"Попытка {attempt + 1}/{self.max_retries} вставки данных не удалась: {e}")
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay)
                    # Попытка переподключения
                    self._connect()
                else:
                    logger.error(f"Не удалось вставить данные после {self.max_retries} попыток в таблицу {table}")
                    raise
            except Exception as e:
    ...
    def command(self, query: str, parameters: Optional[Dict[str, Any]] = None) -> Any:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"Попытка {attempt + 1}/{self.max_retries} выполнения команды не удалась: {e}")
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay)
                    # Попытка переподключения
                    self._connect()
                else:
                    logger.error(f"Не удалось выполнить команду после {self.max_retries} попыток: {query[:100]}...")
                    raise
            except Exception as e:
